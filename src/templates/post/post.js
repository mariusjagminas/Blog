...
const Index = ({ data, intl, intl: { locale }, pageContext: { slug } }) => {
  ...
  const node = data[`${locale}_slugs`].edges.find(
  ...
  const imgFixed = data.node.articleImage
    ? data.node.articleImage.fixed.src
    : null
          ...
          {hasContent ? (
            <>
              <Article
                ...
                image={
                  data.node.articleImage ? data.node.articleImage.fluid : null
                }
                date={data.node.date}
                slug={slug}
              />
...
export const query = graphql`
  query articleTemp($slug: String) {
    ...
    ######
    ######
    pl_slugs: allContentfulArticles(
    ...
    fr_slugs: allContentfulArticles(
    ...
    en_slugs: allContentfulArticles(
