...
const MenuList = styled.ul`
	...
	${({ theme }) => theme.mq.laptop} {
		...
		flex-direction: row;
		align-items: center;
		background: ${({ theme, infooter }) => (infooter ? theme.primaryLight : 'none')};
		color: white;
...
const StyledLink = styled(Link)`
	...
	font-weight: ${({ theme, infooter }) => (infooter ? theme.font.weight.regular : theme.font.weight.bold)};
	padding: 10px;
			...
			<MenuList isMenuOpen={isMenuOpen} {...props}>
				{links.map(link => (
					<MenuItem key={link.title} locale={locale} {...props}>
						<StyledLink activeStyle={{ color: 'rgb(61, 170, 163)' }} {...props} to={link.path}>
							{link.title}
						</StyledLink>
					</MenuItem>
				))}
